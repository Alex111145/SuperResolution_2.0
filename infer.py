...
def run_test(target_model_folder: str):
    ...
    # --- CONFIGURAZIONE MODELLO (Deve combaciare con train.py) ---
    ...
    for t in targets:
        ...
        if test_json_path.exists():
            ...
            with open(test_json_path, 'r') as f:
                data = json.load(f)
                # Aggiunge il nome del target ai dati per riferimento (opzionale)
                for entry in data:
                    entry['target_name'] = t
    ...
    # Crea un file JSON temporaneo per il dataset combinato
